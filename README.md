...
- $_FILES['fieldname']['name'] => 上傳檔案的原始名稱
- $_FILES['fieldname']['type'] => 上傳檔案的檔案類型
- $_FILES['fieldname']['size'] => 上傳的檔案原始大小
- $_FILES['fieldname']['tmp_name'] => 上傳檔案後的暫存資料夾位置
- $_FILES['fieldname']['error'] => 顯示錯誤代碼
